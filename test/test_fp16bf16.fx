...
TEST("fp16bf16.literals", fun() {
    val h = 1.5h
    val b = 2.25bf
    EXPECT_EQ_(float(h), 1.5f)
    EXPECT_EQ_(float(b), 2.25f)
    EXPECT_EQ_(scalar_type(h), Type_F16)
    EXPECT_EQ_(scalar_type(b), Type_BF16)
})
...
TEST("fp16bf16.arith", fun() {
    ...
    EXPECT_EQ_(float(hs), 3.0f)
    EXPECT_EQ_(float(bs), 3.0f)
})
...
TEST("fp16bf16.rounding", fun() {
    ...
    EXPECT_EQ_(as_fp16[0], 257.0f)         // fp16: exact
    EXPECT_EQ_(as_bf16[0], 258.0f)         // bf16: 257 -> 258
    EXPECT_EQ_(as_fp16[1], 1.0996094f)     // fp16 rounding of 1.1
    EXPECT_EQ_(as_bf16[1], 1.1015625f)     // bf16 rounding of 1.1
    EXPECT_EQ_(as_fp16[2], 3.5f)           // exact in both
    EXPECT_EQ_(as_bf16[2], 3.5f)
})
...
TEST("fp16bf16.arrays", fun() {
    ...
    EXPECT_EQ_(float(ha[2]), 3.5f)
    EXPECT_EQ_(float(ba[2]), 3.5f)
    EXPECT_EQ_(size(ha), 3)
    EXPECT_EQ_(size(ba), 3)
})
...
TEST("fp16bf16.inf_nan", fun() {
    ...
    EXPECT_EQ_(string(hinf), "inf");  EXPECT_EQ_(string(binf), "inf")
    EXPECT_EQ_(string(hnan), "nan");  EXPECT_EQ_(string(bnan), "nan")
    // an infinite fp16/bf16 converts back to an infinite float32
    EXPECT_EQ_(float(hinf) > 1e30f, true)
    EXPECT_EQ_(float(binf) > 1e30f, true)
})

// string() of an exact value.
TEST("fp16bf16.string", fun() {
    EXPECT_EQ_(string(1.5h), "1.5")
    EXPECT_EQ_(string(2.0bf), "2.0")
})
