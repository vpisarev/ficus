...
{
    EXPECT_EQ(`Re2.consume_n("Eagle, don't bait! I and Cheburashka, who haven't friends, both want to serve in the artillery.", 0, r"([[:alpha:]]+)")`,
              (true, 5, [| (0, 5) |]))
    EXPECT_EQ(`Re2.consume_n_str("Eagle, don't bait! I and Cheburashka, who haven't friends, both want to serve in the artillery.", 0, r"([[:alpha:]]+)")`,
              (true, 5, [| "Eagle" |]))
    EXPECT_EQ(`Re2.find_and_consume_n_str("The moon is 384467 kilometers distant.", 0, r"(\d+)")`,
              (true, 18, [| "384467" |]))
})
...
TEST("Re2.general_match", fun()
{
    ...
    EXPECT_EQ(`Re2.general_match_str("Hello I'm Cornelius", r"(I.m)", 6, 9, Re2.anchor_t {anchor_both=true})`,
              (true, [| "I'm" |]))
})

TEST("Re2.replace", fun()
{
    EXPECT_EQ(`Re2.replace("If you'll find a Talker bird, please report me, Gromozeka@Chumaroza.org.", r"(\w+)@(\w+)\.(\w+)", r"\1 from \2")`,
              (true, "If you'll find a Talker bird, please report me, Gromozeka from Chumaroza."))
    EXPECT_EQ(`Re2.global_replace("2-12-85-06", r"\d+", r"(\0)")`,
              (true, "(2)-(12)-(85)-(06)"))
})
