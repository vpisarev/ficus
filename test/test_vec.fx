...
TEST("fcvector.access", fun()
{
    val v: vector[int] = []
    EXPECT_EQ_(size(v), 0)
    EXPECT_EQ_(empty(v), true)
    for i <- 0:5 { v.push_back(i*i) }
    EXPECT_EQ_(size(v), 5)
    EXPECT_EQ_(empty(v), false)
    for i <- 0:5 { EXPECT_EQ_(v[i], i*i) }
    EXPECT_EQ_(v.clip[100], 16)
    EXPECT_EQ_(v.clip[-100], 0)
    EXPECT_EQ_(v.zero[100], 0)
    EXPECT_EQ_(v.wrap[-1], 16)
    var sink = 0
    EXPECT_THROWS_(fun() {sink = v[5]}, OutOfRangeError)
    ignore(sink)
...
TEST("fcvector.write", fun()
{
    ...
    EXPECT_EQ_(array(v), [100, 1, 300, 3, 500])
    var sink = 0
    EXPECT_THROWS_(fun() {v[5] = 9; sink = v[5]}, OutOfRangeError)
    ignore(sink)
    ...
    s[0] = "hello"; s[1] = "world"
    EXPECT_EQ_(s[0], "hello")
    EXPECT_EQ_(s[1], "world")
})
...
TEST("fcvector.ops", fun()
{
    val v = vector(3, 7)
    EXPECT_EQ_(size(v), 3)
    EXPECT_EQ_(Vector.capacity(v) >= 3, true)
    EXPECT_EQ_(array(v), [7, 7, 7])
    v.push_back(9)
    v.resize(6, 0)
    EXPECT_EQ_(array(v), [7, 7, 7, 9, 0, 0])
    EXPECT_EQ_(v.back(), 0)
    v.pop_back()
    EXPECT_EQ_(array(v), [7, 7, 7, 9, 0])
    v.clear()
    EXPECT_EQ_(size(v), 0)
    EXPECT_EQ_(empty(v), true)
    ...
    val c = vector([1, 2, 4])
    EXPECT_EQ_(a == b, true)
    EXPECT_EQ_(a == c, false)
    EXPECT_EQ_(a <=> c, -1)
    EXPECT_EQ_(string(a), "[1, 2, 3]")
...
TEST("fcvector.pushpop_edge", fun()
{
    ...
    val v: vector[int] = []
    EXPECT_THROWS_(fun() { v.pop_back() }, OutOfRangeError)
...
TEST("fcvector.slice", fun()
{
    ...
    EXPECT_EQ_(array(v[2:5]), [2, 3, 4])
    EXPECT_EQ_(array(v[:3]), [0, 1, 2])
    EXPECT_EQ_(array(v[7:]), [7, 8, 9])
    EXPECT_EQ_(array(v[:]), array(v))      // flatten = full copy
    EXPECT_EQ_(array(v[::-1]), [9, 8, 7, 6, 5, 4, 3, 2, 1, 0])
    EXPECT_EQ_(array(v[::2]), [0, 2, 4, 6, 8])
    EXPECT_EQ_(array(v[1:8:3]), [1, 4, 7])        // arbitrary stride

    // a slice / flatten is a COPY: mutating it must not touch the source
    val s = v[2:5]
    s[0] = 999
    EXPECT_EQ_(array(s), [999, 3, 4])
    EXPECT_EQ_(array(v[2:5]), [2, 3, 4])

    // complex elements copied on slice
    val w: vector[string] = []
    for x <- ["a", "b", "c", "d"] { w.push_back(x) }
    EXPECT_EQ_(array(w[1:3]), ["b", "c"])
    EXPECT_EQ_(array(w[::-1]), ["d", "c", "b", "a"])
})
...
TEST("fcvector.binomial", fun()
{
    ...
    for i <- 0:size(myvecs) {
        ...
        EXPECT_EQ_(sum(myvecs[i], 0.), expected_sum)
    }
})
...
TEST("fcvector.iteration", fun()
{
    ...
    EXPECT_EQ_(array(v), [100, 101, 4, 9, 16])
    v.push_back(25)
    EXPECT_EQ_(size(v), 6)
})
...
TEST("fcvector.readlock", fun()
{
    ...
    EXPECT_THROWS_(fun() { for x <- v { if x == 2 {v.push_back(99)} } }, VecModifiedError)
    // the lock was released on the exception exit -> mutation works again
    v.push_back(50)
    EXPECT_EQ_(size(v), 11)
...
TEST("fcvector.str", fun()
{
    ...
    EXPECT_EQ_(array(vecstr[::2]), ["a", "def", "klmno", "vwxyz"])
    EXPECT_EQ_(vecstr[::2], vector(["a", "def", "klmno", "vwxyz"]))
    EXPECT_EQ_(array(vecstr[::-1]), ["vwxyz", "pqrstu", "klmno", "ghij", "def", "bc", "a"])
    EXPECT_EQ_(vecstr[::-1], vector(["vwxyz", "pqrstu", "klmno", "ghij", "def", "bc", "a"]))
})
...
TEST("fcvector.append", fun()
{
    ...
    EXPECT_EQ_(array(v), [1, 2, 3, 4, 5, 6, 7])
    ...
    EXPECT_EQ_([for x <- vv {array(x)}], [[1, 2], [3], [4]])

    // push keeps its index-returning semantics
    val p: vector[string] = []
    EXPECT_EQ_(p.push("a"), 0)
    EXPECT_EQ_(p.push("b"), 1)
    EXPECT_EQ_(array(p), ["a", "b"])
...
TEST("fcvector.concat", fun()
{
    ...
    // array of vectors
    EXPECT_EQ_(array(Vector.concat([a, b, d])), [1, 2, 3, 4, 5, 6, 7, 8, 9])
    // empty inputs contribute nothing
    EXPECT_EQ_(array(Vector.concat([a, c, b])), [1, 2, 3, 4, 5])
    // all-empty / empty list -> empty result that is still a real allocated
    // (pushable) vector, not the NULL/default vector
    EXPECT_EQ_(size(Vector.concat([c, c])), 0)
    val e = Vector.concat(([]: vector[int] []))
    EXPECT_EQ_(size(e), 0)
    e.push_back(42)
    EXPECT_EQ_(array(e), [42])
    // vector of vectors
    val vv = vector([a, b, d])
    EXPECT_EQ_(array(Vector.concat(vv)), [1, 2, 3, 4, 5, 6, 7, 8, 9])
    // complex elements (strings) are copied — ASan-checked
    val s1 = vector(["a", "bc"])
    val s2 = vector(["def"])
    EXPECT_EQ_(array(Vector.concat([s1, s2])), ["a", "bc", "def"])
