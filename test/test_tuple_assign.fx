...
TEST("tuple_assign.nested", fun() {
    var x = 0, y = 0, z = 0
    ((x, y), z) = ((10, 20), 30)
    EXPECT_EQ_(x, 10); EXPECT_EQ_(y, 20); EXPECT_EQ_(z, 30)
})
...
TEST("tuple_assign.underscore_component", fun() {
    ...
    (q, _) = eff(7)
    EXPECT_EQ_(q, 7)
    EXPECT_EQ_(log, 1)          // the effectful RHS ran exactly once
})
