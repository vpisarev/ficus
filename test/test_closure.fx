...
TEST("closure.inner_fn_with_fv_var", fun()
{
    ...
    val pf = get_f(1)
    pf(2)
    EXPECT_EQ_(counter, 3)
    pf(3)
    EXPECT_EQ_(counter, 7)
})
...
TEST("closure.kfor_0_no_kfor_fv", fun()
{
    ...
    EXPECT_EQ_(outer_direct(1000), [:: for i <- 0:10 {1100}])
})
...
TEST("closure.kfor_1_fv", fun()
{
    ...
    EXPECT_EQ_(outer_direct_2(1000), [:: for i <- 0:10 {2200 + i}])
})
...
TEST("closure.kfor_1", fun()
{
    ...
    EXPECT_EQ_(outer_direct(1000), [:: for i <- 0:10 {1100 + i}])
})
...
TEST("closure.kfor_2", fun()
{
    ...
    EXPECT_EQ_(outer_direct_k(1000), [:: for i <- 0:10 {2200 + 2 * i + 1}])
})
...
TEST("closure.kfor_3", fun()
{
    ...
    EXPECT_EQ_(outer_inner_val(1000), [:: for i <- 0:10 {1100 + i}])
})
...
TEST("closure.kfor_4", fun()
{
    ...
    EXPECT_EQ_(outer_inner_var(1000), [:: for i <- 0:10 {1100 + i}])
})
...
TEST("closure.kfor_5", fun()
{
    ...
    EXPECT_EQ_(result.1, [:: for i <- 0:10 {1100 + result.0}])
})
...
TEST("closure.object", fun()
{
    ...
    EXPECT_EQ_(closure_object(100), "109;108;107;106;105;104;103;102;101;100;")
})
...
TEST("closure.object_in_tuple", fun()
{
    ...
    EXPECT_EQ_(closure_object(1000), "109;108;107;106;105;104;103;102;101;100;")
})
...
TEST("closure.declare_alias", fun ()
{
    ...
    EXPECT_EQ_(my_own_max_function(2, 3), 3)
})
...
TEST("closure.max_function", fun()
{
    ...
    EXPECT_EQ_(max_6(1), 6)
    EXPECT_EQ_(max_6(7), 7)
})
...
TEST("closure.max_function_depending_on_val", fun()
{
    ...
    EXPECT_EQ_(max_10(1), 10)
    EXPECT_EQ_(max_10(70), 70)
})
...
TEST("closure.self-refencing", fun()
{
    ...
    EXPECT_EQ_(leaf_printer(tree), "[[[[1]], [[2]]], [[[[3]]]], [[[[4]]], [[5]]]]")
    EXPECT_EQ_(leafs, 5)
})
