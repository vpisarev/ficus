...
TEST("oop.interfaces", fun()
{
    ...
    for sh@i <- shapes, (n, a) <- ref_data {
        EXPECT_EQ_(sh.name(), n)
        val sh2 = ((sh :> IClone).clone() :> IShape)
        val (fx, fy) = sh2.get_scale()
        sh2.set_scale(fx*2, fy*2)
        EXPECT_NEAR_(sh2.area(), a*4, 0.01f)
        EXPECT_NEAR_(sh.area(), a, 0.01f)
        fun test_ellipse()
        {
            val e = (sh2 :> Ellipse)
            EXPECT_EQ_(e.cx, e0.cx)
            EXPECT_EQ_(e.cy, e0.cy)
        }
        if i == 0 {
            EXPECT_THROWS_(test_ellipse, TypeMismatchError, "cast (Rect:>IShape):>Ellipse should fail")
        } else {
            EXPECT_NO_THROWS_(test_ellipse, "cast (Ellipse:>IShape):>Ellipse should succeed")
        }
    }
})
