...
TEST("Re.simple", fun()
{
    ...
    EXPECT_EQ_(reg_ho.fullmatch("hello"), true)
    EXPECT_EQ_(reg_e.fullmatch("hello"), false)
    EXPECT_EQ_(reg_ho.prefixmatch("hello").issome(), true)
    EXPECT_EQ_(reg_e.prefixmatch("hello").issome(), false)
    EXPECT_EQ_(reg_e.find("hello").issome(), true)
})
...
TEST("Re.submatch_extraction", fun()
{
    ...
    EXPECT_EQ_(r.find(bards), Some([ (23, 29), (24, 28) ]))
    EXPECT_EQ_(se_reg.find(bards), Some([ (9, 13), (9, 13) ]))
})
...
TEST("Re.find", fun()
{
    ...
    EXPECT_EQ_(letters.prefixmatch(str), Some([ (0, 5) ]))
    EXPECT_EQ_(cword.find_str(str, ignorecase=true), Some(["Cheburashka"]))
    EXPECT_EQ_(digits.find_str("The moon is 384467 kilometers distant."), Some([ "384467" ]))
})

TEST("Re.findall", fun()
{
    val fullstring_re = Re.compile(r".*")
    EXPECT_EQ_(fullstring_re.findall_str("The hooves clattered.\nAs if singing:\n— Crib.\nGrab.\nGrub.\nGruff.\n", multiline=true),
...
TEST("Re.general_match", fun()
{
    ...
    EXPECT_EQ_(me.find_str("Hello, I'm Cornelius"), Some([ "I'm" ]))
})
...
TEST("Re.replace", fun()
{
    ...
    val digits = Re.compile(r"\d+")
    EXPECT_EQ_(email.replace(str, r"\1 from \2"),
        "If you'll find a Talker bird, please report me, Gromozeka from Chumaroza.")
    EXPECT_EQ_(digits.replace("2-12-85-06", r"(\0)"), "(2)-(12)-(85)-(06)")
})
...
TEST("Re.groups", fun()
{
    ...
    EXPECT_EQ_(name, "len_in_pieces")
    EXPECT_EQ_(value, "0x509F934")
})

TEST("Re.exception", fun()
{
    EXPECT_THROWS_(fun() {ignore(Re.compile(r"h.*o["))}, BadArgError)
})
