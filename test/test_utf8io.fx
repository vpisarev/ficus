...
TEST("utf8.length_bytes_vs_codepoints", fun() {
    ...
    EXPECT_EQ("abc".utf8_length(), 3)
    EXPECT_EQ("abc".length(), 3)
    // 2-byte, 3-byte, 4-byte code points: bytes > code points
    EXPECT_EQ("é".utf8_length(), 2);       EXPECT_EQ("é".length(), 1)
    EXPECT_EQ("你好".utf8_length(), 6);     EXPECT_EQ("你好".length(), 2)
    EXPECT_EQ("😀".utf8_length(), 4);       EXPECT_EQ("😀".length(), 1)
    EXPECT_EQ("".utf8_length(), 0)
})

TEST("utf8.from_utf8_decode", fun() {
    // "café" as UTF-8 bytes (é = C3 A9)
    EXPECT_EQ(String.from_utf8([0x63u8, 0x61u8, 0x66u8, 0xC3u8, 0xA9u8]), "café")
    // "你好" (E4 BD A0  E5 A5 BD)
    EXPECT_EQ(String.from_utf8([0xE4u8, 0xBDu8, 0xA0u8, 0xE5u8, 0xA5u8, 0xBDu8]), "你好")
    EXPECT_EQ(String.from_utf8(([]: uint8 [])), "")
})
...
TEST("utf8.framing_invariant", fun() {
    ...
    EXPECT_EQ(msg.length() < nbytes, true)
    ...
    EXPECT_EQ(hi.utf8_length(), 6)
    EXPECT_EQ(hi.length(), 2)
})
