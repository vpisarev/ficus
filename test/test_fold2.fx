...
TEST("fold2.return_in_body", fun() {
    ...
    EXPECT_EQ(sum_until_neg([1, 2, 3, -1, 100]), 6)   // stops at the -1
    EXPECT_EQ(sum_until_neg([1, 2, 3]), 6)            // no negative: full sum
})
...
TEST("fold2.nested", fun() {
    ...
    EXPECT_EQ(total, 99)     // sum over i,j of (i*10+j)
})
...
TEST("fold2.nested_inner_assigns_outer", fun() {
    ...
    EXPECT_EQ(count, 9)      // c incremented 3*3 times
})
...
TEST("fold2.closure_captures_var", fun() {
    ...
    EXPECT_EQ(s, 6)
    EXPECT_EQ([:: for f <- fns { f() }], [:: 6, 6, 6])   // all see the final var
})
