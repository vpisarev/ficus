...
type count_map_t = Hashmap.t[id_t, int]
...
type block_ctx_t =
{
    ...
    bctx_prologue: list[cstmt_t];
    bctx_cleanup: list[cstmt_t];
    bctx_break_used: int;
...
fun gen_main(ismain: bool, mod_names: list[string], loc: loc_t) =
    if !ismain { [] } else {
...
type cexp_map_t = Hashmap.t[id_t, cexp_t]

fun gen_ccode(cmods: list[cmodule_t], kmod: kmodule_t, c_fdecls: ccode_t, mod_init_calls: ccode_t)
{
    ...
    var block_stack: list[ref[block_ctx_t]] = []
    val for_letters = [:: "i", "j", "k", "l", "m" ]
    val fx_status_ = get_id("fx_status")
    var return_used = 0
    var func_dstexp_r: ref[cexp_t?] = ref None
    ...
    fun check_inside_loop(is_break: bool, loc: loc_t)
    {
        ...
        fun check_inside_loop_(s: list[ref[block_ctx_t]])
        {
            | top :: rest =>
    ...
    fun get_dstexp(dstexp_r: ref[cexp_t?], prefix: string, ctyp: ctyp_t, ccode: ccode_t, loc: loc_t) =
        match (ctyp, *dstexp_r) {
    ...
    fun make_vec(cap_exp: cexp_t, et: ktyp_t, dstexp_r: ref[cexp_t?], ccode: ccode_t, loc: loc_t) {
    ...
    fun unpack_fun_args(args: list[id_t, ctyp_t, list[carg_attr_t]], rt: ctyp_t, is_nothrow: bool) {
    ...
    fun decl_plain_arr(arr_id: id_t, elem_ctyp: ctyp_t, arr_data: list[cexp_t], ccode: ccode_t, loc: loc_t) =
        match arr_data {
        | [] => (make_nullptr(loc), ccode)
        | _ =>
    ...
    fun make_make_arr_call(arr_exp: cexp_t, shape: list[cexp_t], data: list[cexp_t],
    ...
    fun make_make_vec_call(vec_exp: cexp_t, data: list[cexp_t],
                            ccode0: ccode_t, lbl: cexp_t, loc: loc_t)
    {
    ...
    fun maybe_deref_fun_arg(idx: int, arg: cexp_t, t: ctyp_t, flags: list[carg_attr_t], loc: loc_t) =
    ...
    fun add_size_eq_check(check_list: list[cexp_t], ccode: ccode_t,
    ...
    fun add_size_post_check(check_list: list[cexp_t], ccode: ccode_t,
                            lbl: cexp_t, loc: loc_t) =
        match check_list {
        | e0 :: rest =>
    ...
    fun compute_for_ndims(for_idx: int, nfors: int, idoml: list[id_t, dom_t], for_loc: loc_t) =
    ...
    */
    fun process_for(lbl: cexp_t, idoml: list[id_t, dom_t], at_ids: list[id_t],
                    for_idx: int, nfors: int, ndims: int, dims_ofs: int,
                    nested_e_idoml: list[kexp_t, list[id_t, dom_t], list[id_t]],
                    init_ccode: ccode_t, loc: loc_t)
    {
        ...
        fun get_iter_id(k: int, at_ids: list[id_t], prefix: string) =
            match at_ids {
        ...
        var list_exps = ([]: list[cexp_t]), i_exps = ([]: list[cexp_t]),
            n_exps = ([]: list[cexp_t]), for_checks = [],
        ...
        for (iter_val_i, dom_i)@k <- idoml {
            ...
            val (lists_i, i_exps_n, n_exps_n, for_checks_n, incr_exps_n, init_checks_n,
                ...
                (list[cexp_t], list[cexp_t], list[cexp_t], list[cexp_t], list[cexp_t], list[cexp_t], ccode_t, ccode_t, list[id_t, cexp_t, val_flags_t], list[cexp_t]) =
            match dom_i {
    ...
    fun decl_for_body_elems(body_elems: list[id_t, cexp_t, val_flags_t], body_ccode: ccode_t) =
    ...
    fun process_cases(cases: list[list[kexp_t], kexp_t], dstexp_r: ref[cexp_t?],
                      ccode: ccode_t, is_catch_case: bool, kloc: loc_t)
    {
        ...
        for (checks_i, action_i) <- cases {
            ...
            | _ =>
                ...
                val ai_ccode: list[cstmt_t] = ai_ccode + prologue
    ...
    */
    fun kexp2cexp(kexp: kexp_t, dstexp_r: ref[cexp_t?], ccode: ccode_t)
    {
        ...
        | KExpMap (e_idoml_l, body, flags, _) =>
            ...
            */
            ...
            fun form_map(pre_map_ccode: ccode_t, for_idx: int,
                         e_idoml_l: list[kexp_t, list[id_t, dom_t], list[id_t]],
                         prev_i_exps: list[cexp_t], prev_n_exps: list[cexp_t])
            {
                ...
                | [:: (body, [], [])] =>
                    ...
                    if !is_parallel_map {
                        ...
                        var new_dst_data = [], decl_dstptr_ccode_all = ([] : list[cstmt_t])
...
fun gen_ccode_all(kmods: list[kmodule_t]): list[cmodule_t]
{
