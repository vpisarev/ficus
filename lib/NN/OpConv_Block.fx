...
void _fx_conv_update_block_f16( int np, const void* a_, const void* b_, void* c_, int ldc, bool init_c )
{
    ...
    for( int p = 0; p < np; p++, a += FX_CONV_MR_FP16, b += FX_CONV_NR_FP16 )
    {
    ...
    for( int p = 0; p < np; p++ )
    {
        ...
        {
            float ai = FX_FLOAT(a[FX_CONV_MR_FP16*p + i]);
            for( int j = 0; j < FX_CONV_NR_FP16; j++ )
                cbuf[i*FX_CONV_NR_FP16+j] += FX_FLOAT(b[FX_CONV_NR_FP16*p + j])*ai;
        }
    }
    if (!init_c) {
        for(int i = 0; i < FX_CONV_MR_FP16; i++) {
