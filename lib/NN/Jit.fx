                             ...
                             int ntasks)
{
    ...
    calc_dwc_algs_limits_f32(ctx, &algs_limits_, (tail_task_num ? NCtask_ + 1 : NCtask_), Hi, Wi, Hk, Wk, H0, W0, pad_top, pad_left, pad_bottom, pad_right, stride_x, stride_y, dilation_x, dilation_y);
    if(tail_task_num)
        calc_dwc_algs_limits_f32(ctx, &algs_limits_tail, NCtask_, Hi, Wi, Hk, Wk, H0, W0, pad_top, pad_left, pad_bottom, pad_right, stride_x, stride_y, dilation_x, dilation_y);
                             ...
                             int ntasks)
{
    ...
    struct dwc_algs_limits algs_limits_;
    struct dwc_algs_limits algs_limits_tail;
    calc_dwc_algs_limits_f16(ctx, &algs_limits_, (tail_task_num ? NCtask_ + 1 : NCtask_), Hi, Wi, Hk, Wk, H0, W0, pad_top, pad_left, pad_bottom, pad_right, stride_x, stride_y, dilation_x, dilation_y);
    if(tail_task_num)
        calc_dwc_algs_limits_f16(ctx, &algs_limits_tail, NCtask_, Hi, Wi, Hk, Wk, H0, W0, pad_top, pad_left, pad_bottom, pad_right, stride_x, stride_y, dilation_x, dilation_y);
    #pragma omp parallel for num_threads(ntasks)
    for (int task_id = 0; task_id < ntasks; task_id++)
    {
        int NC0 = task_id * NCtask_ + (task_id < tail_task_num ? task_id : tail_task_num);
...
#if _FX_NN_ENABLE_FP16
    conv->jit_func_f16 = (void*)generate_dwc_f16(ctx, kh, kw, padding_top, padding_left, padding_bottom, padding_right, stride_x, stride_y, dilation_x, dilation_y, activation_type, alpha);
#endif //_FX_NN_ENABLE_FP16
#endif
}
