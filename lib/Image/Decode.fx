...
@private fun imread_(filename: string, bpp: int, desired_channels: int): uint8 [,]
@ccode {
    ...
    if (data && channels == desired_channels) {
        int_ size[] = {height, width};
        size_t elemsize = channels*(bpp == 8 ? 1 : 2);
        fx_status = fx_make_arr(2, size, elemsize, 0, 0, data, fx_result);
    } else {
        FX_SET_EXN_FAST(FX_EXN_IOError);
    }
    if (data)
