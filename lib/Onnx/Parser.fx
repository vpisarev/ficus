...
{
    val some_attrs = [ Ast.Attr {name="size", v=Ast.AttrInt(1i64)}, Ast.Attr {name="url", v=Ast.AttrString("http://a.b.c")}]
    val some_node = Ast.Node {name="n", op="conv", inputs=["0"], outputs=["1"], attrs=some_attrs}
    val some_dim = Ast.DimValue(1i64)
    ...
    val some_opset = Ast.opset_t {version=1i64, domain=""}

    ([some_graph], [some_node], [some_tensor],
    some_attrs, [some_vi], [some_dim], ["abc"], [0.f], [0i64], [some_opset], [("key", "value")])
}
