...
So, what are the essential features that are planned for Ficus 1.0?
  ...
  - automatic memory management; the current implementation of Ficus uses reference counting with various optimizations to reduce unncessary increments/decrements  and also to avoid atomic operations whenever possible. Comparing to a full-scale Garbage Collector, Reference Counting let us reduce the runtime overhead (especially the sudden delays incurred by some not very sophisticated GC algorithms) and make the apps less memory-hungry and more cache-friendly. It's also makes interaction with 3rd-party C libraries and the inline C code (see further) much more simple .
