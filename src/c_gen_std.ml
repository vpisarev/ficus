...
let init_std_names () =
    curr_exn_val := -1024;
    std_fx_alloc := gen_std_fun "fx_alloc" (CTypSize_t :: []) CTypVoidPtr;
    std_fx_free := gen_std_fun "fx_free" (CTypVoidPtr :: []) CTypVoid;
    ...
    let make_arr_args0 = CTypSize_t :: std_fx_free_elem_t :: std_fx_copy_elem_t :: (make_ptr CTypAnyArray) :: [] in
    ...
    std_fx_free_arr := gen_std_fun "fx_free_arr" ((make_ptr CTypAnyArray) :: []) CTypVoid;
    std_fx_copy_arr := gen_std_fun "fx_copy_arr"
        ((make_const_ptr CTypAnyArray) :: (make_ptr CTypAnyArray) :: []) CTypVoid;;
